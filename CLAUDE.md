...
- **Tests** are JUnit 5 **system tests** — every test bundle needs an OSGi runtime, so run them as an **Eclipse JUnit Plug-in Test** in the IDE, or via `./mvnw verify`. They all live in a `…test.system` package and share one rule: _nothing is substituted_. The subject is the registered `@Component` out of the service registry, the broker is the real e4 broker, the files land in the real instance location, and the SWT tests run on a real `Display`. A call is judged by the state it leaves behind and the events that left the bus, never by what a collaborator was seen being told.
  - `de.tonsias.basis.osgi.test` exports **`de.tonsias.basis.osgi.test`** (`x-friends` to the logic and delta-view test bundles) with the shared harness:
    - **`ProductRuntime`** — `start()` primes the runtime and returns the root instanz, plus accessors for every service, `flushDeltas()`, and helpers that read objects back off disk. Start here in `@BeforeEach`.
    - **`E4ServiceContext.prime()`** — what `ProductRuntime.start()` calls. `IEventBrokerBridge` and `IDeltaService` are not plain DS components; they come from `IContextFunction`s that only run when an `IEclipseContext` is asked for their key, and `ChangePropagationListener` is contributed as an e4 _addon_ in `Application.e4xmi`. In the product the workbench triggers all three; headless nothing does, so without priming `InstanzServiceImpl`'s mandatory `@Reference IEventBrokerBridge` stays unsatisfied and the component never activates.
    - **`EventRecorder`** — subscribes to the delta topics on the real bus and collects what passes, so a test can assert on a whole propagation chain. Its `awaitCount` / `awaitTopic` cover `Type.POST` and work that runs in a `Job`.
  - The runtime is **shared by every test in a bundle** — one Equinox, one root instanz, one key sequence, one delta log. Build your own subtree below the root rather than assuming an empty model, and hand the delta log back empty with `ProductRuntime.flushDeltas()` in `@AfterEach`. Tests write real files to the instance location (`target/work/data`), which is cleaned per run.
  - Where the shared singleton makes a scenario unreachable — `KeyServiceImpl`'s counter is the case — build a second **real** instance on a preference node of its own rather than reaching for a stand-in.
...
Step 6 places tests in the `…test.system` package of the test bundle matching the layer under test. All run inside OSGi against the registered services — start from `ProductRuntime`, never construct the subject with its collaborators replaced. A new bundle's internals need `x-friends` on its `Export-Package` before its test bundle can see them.
