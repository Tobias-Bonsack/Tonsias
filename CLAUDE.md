...
**Model.** `IInstanz` is the single tree node type: own key, parent key, child keys, and per-`IValueType` `BiMap<valueKey, name>` of attributes. Attributes are `IValue` objects in their own files, and there are **ten types: five contents (`ValueContentType`) times two kinds** — `SingleValueType` holds one content, `MultiValueType` an ordered list of it without duplicates. Both enums implement `IValueType`, `ValueTypes` is the two of them together, and `ValueContentRules` holds every accepts/convert rule once, so a single value and the list of the same content cannot drift apart. Anything that only differs by content switches over the five; only what really differs asks `isMulti()`. **Everything is referenced by string key, never by object reference** — services resolve through a cache and fall back to disk. Key `"0"` is the root by convention. Keys are a base-36 counter (`KeyServiceImpl.KEYCHARS`), **lower case only** (file names on case-insensitive filesystems) and **sorted ascending** (binary search).
...
- Topics and payload records live together in `…intf.non.service.*EventConstants`. **Adding a topic means adding its payload record next to it, registering it in `KNOWN_DELTA`, and giving it a `case` in the matching `DeltaServiceImpl.handle*Events`** — a topic in `KNOWN_DELTA` without a branch there throws inside `saveDeltas`, which clears the log in a `finally`, so the whole batch is lost without a trace.
