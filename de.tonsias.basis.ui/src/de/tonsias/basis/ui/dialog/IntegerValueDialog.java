...
public class IntegerValueDialog extends Dialog {

	private SingleIntegerValue _value;
	...
	private void createInstanzPart(Composite composite) {
		...
		String name = _instanz.getSingleValues(SingleValueTypes.SINGLE_INTEGER).get(_value.getOwnKey());
	...
	private void createSingleValuePart(Composite composite) {
		...
		_valueText = TextFactory.newText(SWT.None).text("" + _value.getValue()).enabled(true)//
				.onVerify((e) -> {
					if (!_valueText.getText().isEmpty() && !_valueText.getText().matches("\\d*")) {
						e.doit = false;
					}
				}).create(composite);
	...
	protected void okPressed() {
		_value.setValue(Integer.valueOf(_valueText.getText()));
		_instanz.getSingleValues(SingleValueTypes.SINGLE_INTEGER).put(_value.getOwnKey(), _nameText.getText());
		super.okPressed();
	}
